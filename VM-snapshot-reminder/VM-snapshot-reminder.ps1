...
Function SnapMail ($Mailto, $snapshot)
{
   ...
   $msg.From = $MailFrom
   $msg.to.Add($Mailto)
   $msg.Bcc.Add("John.yang@activenetwork.com,Shayne.Niu@activenetwork.com,Cookies.Zhang@activenetwork.com,Mary.Xu@activenetwork.com,paul.hu@activenetwork.com,Andy.Burkett@activenetwork.com,Nandish.Parmar@activenetwork.com")
   #$msg.cc.Add("john.yang@activenetwork.com")
 
   $msg.Subject = "Snapshot Reminder"
 
$MailText = @"
This email is generated by system automatically.  Please log a Service Now ticket to “Shared Services - SOI” with any questions.
...
The datastore`s performance and capacity will be impacted when the you keep the snapshots.

Please submit a backup ticket for this server, if you want to rollback the whole system status.

Please delete your snapshot if it's not used any more.
...
Function Snap_delete_Mail ($Mailto, $snapshot)
{
   $msg = new-object Net.Mail.MailMessage
   $smtp = new-object Net.Mail.SmtpClient($smtpServer)
   $msg.From = $MailFrom
   $msg.to.Add($Mailto)
   $msg.Bcc.Add("John.yang@activenetwork.com,Shayne.Niu@activenetwork.com,Cookies.Zhang@activenetwork.com,Mary.Xu@activenetwork.com,paul.hu@activenetwork.com,wesley.fan@activenetwork.com,Andy.Burkett@activenetwork.com,Nandish.Parmar@activenetwork.com")
   #$msg.Bcc.Add("John.yang@activenetwork.com")
   $msg.Subject = "Snapshoot deletion Reminder"
 
$MailText = @"
This email is generated by system automatically,please contact us if you have any questions about this.

This is a reminder that you have a snapshot active on virtual machine $($snapshot.VM), which was taken on $($snapshot.Created) and overed $($Delete_days) days.

The snapshot $($snapshot.Name) of $($snapshot.VM) has been automatically deleted.
...
$CredsFile = "D:\schedule_jobs\VM-snapshot-reminder\key\vmadmin.key"
#$CredsFile = "keys\vmadmin.key"
...
Foreach ($VC in $VCs ) {
 ...
 Connect-VIServer -Server $VC -Credential $Cred
    ...
    $VMs = Get-VM
	$whitelist = @() 
	$exception = get-content D:\schedule_jobs\VM-snapshot-reminder\whitelist.txt
	$whitelist += $exception 
	$n= ($whitelist |measure).count
    if ($n -gt 0)	{	
		for($i=0;$i -le $n ;)
		{
		$VMs= $VMs|where{$_.name -notlike $whitelist[$i]}
		$i++
		}
}

foreach ($snap in ($VMs| Get-Snapshot | Where {$_.Created -lt ((Get-Date).AddDays(-$Warning_days))})){
   $SnapshotInfo = Get-SnapshotExtra $snap
   $time_stamp = get-date
   $overdays = ($time_stamp - $SnapshotInfo.Created).Days
   #auto remaind over $warning days snapshot 
   if ($overdays -lt $Delete_days){
   ...
   #auto remove over $delte_days
   else {
   write-host $SnapshotInfo
   Remove-Snapshot -Snapshot $SnapshotInfo -Confirm:$false -RunAsync
   $mail_domain = "@activenetwork.com"
   $mail_user =  Find-User $SnapshotInfo.Creator
   $mailto = (($mail_user.Properties.cn -replace (" ","."))|Out-String).ToString().Trim() + $mail_domain
   Snap_Delete_Mail $mailto $SnapshotInfo
   }
}
