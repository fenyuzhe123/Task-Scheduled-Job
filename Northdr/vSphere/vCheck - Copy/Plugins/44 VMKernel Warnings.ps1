...
foreach ($VMHost in ($HostsViews)){
	...
	if ($product -eq "embeddedEsx" -and $VIVersion -lt 5){
		$Warnings = (Get-Log -vmhost ($VMHost.name) -Key messages -ErrorAction SilentlyContinue).entries |where {$_ -match "error" -and $_ -match "vmkernel"}
		if ($Warnings -ne $null) {
			$VMKernelWarning = @()
			$Warnings | % {
				...
				$Details.Google = "<a href='http://www.google.com/search?q=$Message' target='_blank'>Click Here</a>"
	...
	{
		$Warnings = (Get-Log –VMHost ($VMHost.Name) -Key vmkernel -ErrorAction SilentlyContinue).Entries | where {$_ -match "error"}
		if ($Warnings -ne $null) {
			$VMKernelWarning = @()
			$Warnings | % {
				$Details = "" | Select-Object VMHost, Message, Length, KBSearch, Google
				...
				$Details.Google = "<a href='http://www.google.com/search?q=$($Details.Message)' target='_blank'>Click Here</a>"
				$VMKernelWarning += $Details
				}
			$VMKernelWarnings += $VMKernelWarning | Sort-Object -Property Length -Unique |select VMHost, Message, KBSearch, Google			
		}
