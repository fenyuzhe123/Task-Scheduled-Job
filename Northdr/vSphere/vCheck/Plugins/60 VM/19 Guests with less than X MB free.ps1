...
ForEach ($VMdsk in $SortedVMs){
	...
	Foreach ($disk in $VMdsk.Guest.Disk){
		if ((([math]::Round($disk.Capacity / 1MB)) -gt $MBDiskMinSize) -and (([math]::Round($disk.FreeSpace / 1MB)) -lt 1024)){
...
$Title = "Guests with less than 1024 MB"
$Header = "VMs with less than 1024 MB : $(@($MyCollection).count)"
$Comments = "The following guests have less than 1024 MB Free, if a guest disk fills up it may cause issues with the guest Operating System"
$Display = "Table"
$Author = "Alan Renouf"
$PluginVersion = 1.1
