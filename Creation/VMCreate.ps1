...
$VIEvent = Get-VIEvent -maxsamples 30000 -Start (get-date).AddDays(-1)
...
$Pre = "<img src='cid:top.png'><P>Created VM:</p>"
$Post = "<br><br><img src='cid:bottom.png'>"
  ...
  AttachImage
  $msg.isbodyhtml = $true
  $msg.Body = $OutputCreatedVMs|ConvertTo-Html -Head $Header -PreContent $Pre -PostContent $post  | Set-AlternatingRows -CSSEvenClass even -CSSOddClass odd
    
  if($OutputCreatedVMs.Count -ne 0){
