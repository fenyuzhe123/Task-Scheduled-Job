...
$HAIssues += $Clusters | Where-Object {$_.Name -notmatch $ClustersDoNotInclude -and -not $_.HAEnabled} |
  Select-Object @{Name="Cluster";Expression={$_.Name}},@{Name="Configuration Issue";Expression={$pLang.HADisabled}},@{Name="Datacenter";Expression={(get-datacenter -cluster $_).name}},@{Name="Host Number";Expression={$_.ExtensionData.Summary.NumHosts}}
...
$HAIssues += $clusviews | where {$_.Name -notmatch $ClustersDoNotInclude -and $_.Configuration.DasConfig.HostMonitoring -eq "disabled"} |
   Select-Object @{Name="Cluster";Expression={$_.Name}}, @{N="Configuration Issue";E={$pLang.HAMonDisabled}},@{Name="Datacenter";Expression={(get-datacenter -cluster $_).name}},@{Name="Host Number";Expression={$_.ExtensionData.Summary.NumHosts}}
...
$HAIssues += $Clusters | Where-Object {$_.Name -notmatch $ClustersDoNotInclude -and -not $_.HAAdmissionControlEnabled} |
  Select-Object @{Name="Cluster";Expression={$_.Name}},@{Name="Configuration Issue";Expression={$pLang.HAACDisabled}},@{Name="Datacenter";Expression={(get-datacenter -cluster $_).name}},@{Name="Host Number";Expression={$_.ExtensionData.Summary.NumHosts}}
